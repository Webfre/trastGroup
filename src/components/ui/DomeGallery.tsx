...
type DomeCssVars = CSSProperties & Record<`--${string}`, string | number>;
...
export function DomeGallery({
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver(entries => {
      ...
      const enlargedOverlay = viewerRef.current?.querySelector('.dg-enlarge') as HTMLElement;
  ...
  useGesture(
    {
      onDragStart: ({ event }) => {
        ...
        stopInertia();
        const evt = event as unknown as globalThis.PointerEvent;
  ...
  useEffect(() => {
    ...
    const close = () => {
      ...
      const refDiv = parent.querySelector('.dg-item__image--reference') as HTMLElement | null;
            ...
            {items.map((it, i) => (
              ...
              >
                <div
                  ...
                  role="button"
                  tabIndex={0}
                  aria-label={it.alt || 'Открыть изображение'}
                  onClick={onTileClick}
